 ...
 */
  inline bool CartesianPositionController::doUpdate(const ros::Time& /*time*/, const ros::Duration& period)
  {
    ...
    rosdyn::getFrameDistance(this->chainNonConst().getTransformation(pos_sp),T_base_setpoint_,distance);
    if (distance.norm()>1e-5)
      CNR_WARN_THROTTLE(this->logger(),1.0,"ik distance = " << distance);
